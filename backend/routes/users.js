...
router.get("/public-trainers", async (req, res) => {
  try {
    ...
    // Optional specialty filter (single or CSV or repeated param)
    const specialtyQ = req.query.specialty;
    if (specialtyQ) {
      let specs = [];
      if (Array.isArray(specialtyQ)) {
        specs = specialtyQ.map(s => String(s).trim()).filter(Boolean);
      } else {
        specs = String(specialtyQ).split(',').map(s => s.trim()).filter(Boolean);
      }
      if (specs.length > 0) {
    ...
    const [trainers, total] = await Promise.all([
      User.find(filter)
        .select(
          "fullName bio specialties sessionRate availability profileImage"
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
...
router.get("/public-trainers/:id", async (req, res) => {
  try {
    const trainer = await User.findById(req.params.id).select(
      "fullName bio specialties sessionRate availability profileImage approvalStatus role"
    );
    if (
      !trainer ||
...
router.put(
  ...
  [
    ...
    body("fitnessGoal")
      .optional()
      .isIn(["weight loss", "muscle gain", "endurance", "flexibility"])
    ...
    body("sessionCapacity")
      .optional()
      .isInt({ min: 1 })
      .withMessage("Session capacity must be at least 1"),
  ...
  async (req, res) => {
    try {
      ...
      if (user.role === "trainer") {
        ...
        if (req.body.sessionRate !== undefined)
          user.sessionRate = req.body.sessionRate;
        if (req.body.sessionCapacity !== undefined)
          user.sessionCapacity = req.body.sessionCapacity;
...
router.post(
  ...
  [
    ...
    body("sessionRate")
      ...
      .withMessage("Session rate must between 500 - 5000"),
    body("sessionCapacity")
      .optional()
      .isInt({ min: 1 })
      .withMessage("Session capacity must be at least 1"),
    body("availability.days")
      .optional()
      .isArray()
      .withMessage("Days must be an array if provided"),
    body("availability.days.*")
      .optional()
      .isIn([
        "Monday",
...
router.put("/trainers/:id/approve", auth, adminAuth, async (req, res) => {
  try {
    ...
    if (trainer.approvalStatus !== "pending") {
      return res
        .status(400)
        .json({ message: "Trainer registration is not pending" });
    }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (req.body.specialties !== undefined)
        trainer.specialties = req.body.specialties;
      if (req.body.sessionRate !== undefined)
        trainer.sessionRate = req.body.sessionRate;
      if (req.body.sessionCapacity !== undefined)
        trainer.sessionCapacity = req.body.sessionCapacity;
      if (req.body.availability !== undefined)
        trainer.availability = req.body.availability;
      if (req.body.profileImage !== undefined)
        trainer.profileImage = req.body.profileImage;
